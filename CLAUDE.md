...
*El Secreto de Castronegro* — a free, deterministic interactive-fiction engine for *La Llamada de Cthulhu* 7e (Call of Cthulhu), entirely in Spanish. No AI, no server, no accounts: the engine runs 100% in the browser, the event log lives in the player's IndexedDB, and the prose is hand-written content per adventure. There *was* an LLM Keeper that narrated with Claude; it was fully removed (see `docs/ROADMAP.md` §3.2-undecies and `README.md`) — "Keeper" in the code now means the deterministic offline resolver in `src/keeper/`, not an AI.

Read `README.md` first — it explains the project's thesis (the engine owns state, nothing else can), the verifiable RNG chain, and the canon/disclosure system in more depth than is repeated here. `docs/CANON.md` holds the campaign's canon bible. `docs/ROADMAP.md` is the running design log — every feature/bugfix this project has shipped gets a dated, reasoned entry there; skim it before assuming something is unbuilt. `docs/` also holds `CANON-MODULO-ORIGINAL.md` (the bridge to the original published module) and lore-export docs for reading the campaign's story outside the code (`castronegro-resumenes.md`, `castronegro-aventuras.md`, `castronegro-especificaciones.md`); `docs/legacy/` holds superseded design documents kept only as historical record — never cite them as current canon or architecture.
