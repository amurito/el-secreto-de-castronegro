...
async function main() {
  ...
  {
    const id = await createCampaign(AGUA_QUIETA, 'FOBIA-DADOS', 'b'.repeat(64));
  ...
  {
    const id = await createCampaign(AGUA_QUIETA, 'ALJIBE-FOBIA', 'd'.repeat(64));
    ...
    const fobia = final.investigator.conditions.find((c) => c.name === 'Horror a las superficies quietas');
  ...
  {
    const id = await createCampaign(LA_LEGUA, 'ALAMBRADO-MANIA', 'e'.repeat(64));
    ...
    if (!escena) {
      check('se encuentra la escena de caminar', false, `${t.state.board.clues.length} pistas`);
    } else {
      const out: string[] = [];
      ejecutarEscena(t, escena, i, out, forzando(t, 'fumble'));
      await t.commit();
      const final = await Turn.open(id);
      const mania = final.investigator.conditions.find((c) => c.name === 'Compulsión de contar');
