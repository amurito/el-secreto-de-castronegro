...
try {
  ...
  node(TSX, ['src/prueba-invierno-debido.ts']);
  node(TSX, ['src/prueba-sueno-debido.ts']);
  ...
  node(TSX, ['src/prueba-fobias.ts']);
  node(TSX, ['src/prueba-permeabilidad.ts']);
  node(TSX, ['src/prueba-combate.ts']);
  node(TSX, ['src/prueba-condiciones.ts']);
  node(TSX, ['src/prueba-carga-contenido.ts']);
  console.log('      motor, acción libre, opciones, desenlaces, social, Umbral, desarrollo, desacople, las cuatro primeras aventuras, El Invierno Debido, El Sueño Debido, el encadenado, la creación, el criterio de tiradas, la auditoría de alcanzabilidad, el tratamiento, crítico/pifia, Cordura, fobias/manías, la presión de tiempo, las reglas de combate, el lenguaje de condiciones y la carga de contenido en verde');
